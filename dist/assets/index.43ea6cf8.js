...
const firebaseConfig = {
  apiKey: "AIzaSyAx3-kq5byKWvlus3lLwn6aHeXFDfU_rBo",
  authDomain: "insta-pet-s.firebaseapp.com",
  projectId: "insta-pet-s",
  storageBucket: "insta-pet-s.appspot.com",
  messagingSenderId: "145661924517",
  appId: "1:145661924517:web:39a6cde981f0a4c156a94c"
};
const firebaseApp = initializeApp(firebaseConfig);
const auth = getAuth(firebaseApp);
const db = wh(firebaseApp);
function cadastrar(name2, email, senha) {
  const auth2 = getAuth(firebaseApp);
  return createUserWithEmailAndPassword(auth2, email, senha).then(() => updateProfile(auth2.currentUser, {
    displayName: name2
  }));
}
function loginUser(email, senha) {
  const auth2 = getAuth(firebaseApp);
  return signInWithEmailAndPassword(auth2, email, senha);
}
function loginGoogle() {
  const provider = new GoogleAuthProvider();
  return signInWithPopup(auth, provider);
}
function mantemLogado(callback) {
...
const login = () => {
  ...
  btnGoogle.addEventListener("click", (e) => {
    ...
    loginGoogle().then(() => {
      ...
      alert("Login n\xE3o foi poss\xEDvel tente novamente.");
    });
  });
  btnLogin.addEventListener("click", (e) => {
    e.preventDefault();
    loginUser(email.value, password.value).then((userCredential) => {
...
const cadastro = () => {
  ...
  <div class= "verticalCadastro"></div> 
   <form class = caixaTextos>
    ...
    <br><label for="usuario" class="texto">Crie seu nome de usu\xE1rio</label><br>
    <input type="text" id="usuario" class= "inputs" required placeholder="Usus\xE1rio">
  ...
  btn.addEventListener("click", (e) => {
    e.preventDefault();
    cadastrar(usuario.value, email.value, senha.value).then(() => {
